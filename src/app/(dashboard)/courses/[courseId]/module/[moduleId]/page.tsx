...
const ModuleDetail: React.FC = () => {
  ...
  const { course, completedLessons, toggleLessonCompleted, notes, updateNote } =
    useCourseContext();
  ...
  const handleGenerateQuiz = async () => {
    ...
    try {
      if (courseId === "power-bi") {
        ...
        const result = await generateQuiz(module.title, lessonTitles, courseId);
        console.log("Resultado da API (Quiz):", result);
        if (result && Array.isArray(result)) {
          setQuizQuestions(result);
          setShowGenericQuiz(true);
        } else {
          console.error(
            "Erro: A API não retornou o JSON de quiz esperado.",
            result,
          );
        }
      }
        ...
        <ul className="space-y-3">
          {module.lessons.map((lesson) => (
            ...
            >
              ...
              >
                <span
                  className={`${completedLessons.has(lesson.id) ? "line-through text-gray-500" : ""}`}
                >
                  {lesson.title}
                </span>
              </Link>
      ...
      {isModuleCompleted && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md space-y-6">
          ...
          <p className="text-center text-gray-600 dark:text-gray-400">
            Utilize as ferramentas de IA abaixo para revisar e fixar o seu
            aprendizado.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              ...
              className="p-4 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:bg-gray-400 flex flex-col items-center"
            >
              {isLoading.summary ? (
          ...
          {/* QUIZ PRÁTICO PADRÃO PARA OUTROS CURSOS */}
          {showGenericQuiz && <GenericQuiz questions={quizQuestions} />}
        </div>
      )}
    </div>
  );
